...
class _MapScreenState extends State<MapScreen> {
  ...
  // Route fetching disabled - red line was not accurately locating road routes
  Future<void> _fetchRoute(TouristLocation loc) async {
    // Disabled to prevent inaccurate route visualization
  }

  Set<Polyline> _buildRoutePolyline(List<LatLng> points, TouristLocation loc) {
    Color routeColor = const Color(0xFF2196F3);
    final relevantIds = TrafficService.destinationRoads[loc.id] ?? [];
    if (relevantIds.isNotEmpty) {
      final segs = _trafficSegments.where((s) => relevantIds.contains(s.id));
      if (segs.isNotEmpty) {
        final hasHeavy = segs.any((s) => s.level == TrafficLevel.heavy);
        final hasModerate = segs.any((s) => s.level == TrafficLevel.moderate);
        if (hasHeavy) {
          routeColor = const Color(0xFFE53935);
        } else if (hasModerate) {
          routeColor = const Color(0xFFFFA726);
        } else {
                ...
                itemBuilder: (_, idx) {
                  ...
                  return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          forecast.dateTime.hour.toString().padLeft(2, '0'),
